...
class DINOv2SegDecoderWithScore(nn.Module):
    def __init__(self, in_dim=768, out_size=56, num_heads=4, num_layers=6, feat_H=16, feat_W=16):
        super().__init__()
...
class origin_DINOv2SegDecoderWithScore(nn.Module):
    def __init__(self, in_dim=768, out_size=56, num_heads=4, num_layers=2, feat_H=16, feat_W=16):
        super().__init__()
        self.out_size = out_size
        self.encoder_conv = nn.Conv2d(in_dim, 256, kernel_size=1)
        self.relu = nn.ReLU(inplace=True)
...
def train_ddp(rank, world_size):
    ...
    print('into training.....')
    if rank == 0:
        log_file = f"/netscratch/zlu/CutvLER/output/vit_dinobase_imagenet/train_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
    ...
    teacher_classifier = TeacherClassifier(feat_dim=768).to(device)
    ...
    #####read checkpoint#########
    checkpoint_path =None #"/netscratch/zlu/CutvLER/output/vit_dinobase_2/student_teacher_epoch1.pt"
    start_epoch = 0 # 从第5轮继续
    ...
    for epoch in range(start_epoch, total_epoch):
        ...
        for batch in train_loader:
            ...
            masks_resized = masks_resized.clamp(0, 1)
        ...
        if rank == 0:
            ...
            if (epoch + 1) % 1 == 0:
                ...
                }, f'/netscratch/zlu/CutvLER/output/vit_dinobase_imagenet/student_teacher_epoch{epoch+1}.pt')
...
def eval_ddp(rank, world_size):
    ...
    set_seed(42 + rank)
    if rank == 0:
        log_file = f"/netscratch/zlu/CutvLER/output/vit_dinobase3/val_log_21.log"
        logging.basicConfig(
            level=logging.INFO,
            format='%(asctime)s - %(levelname)s - %(message)s',
            handlers=[
    ...
    logger = logging.getLogger()
    student_model = DINOv2SegDecoderWithScore().to(device)
    teacher_backbone = Dinov2Model.from_pretrained("facebook/dinov2-base").to(device)
    teacher_backbone.eval()

    checkpoint = torch.load("/netscratch/zlu/CutvLER/output/vit_dinobase3/student_teacher_epoch21.pt", 
    ...
    val_sampler = DistributedSampler(val_dataset, num_replicas=world_size, rank=rank, shuffle=False)
    val_loader = DataLoader(val_dataset, batch_size=128, sampler=val_sampler, num_workers=2, pin_memory=True)
