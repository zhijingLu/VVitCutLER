...
def save_batch_eig_vecs(batch_files, eigenvectors, save_dir):
    save_directory = '/'+"/".join(save_dir.split("/")[-2:])+'/'
    #print('save_directory',save_directory)
    #/netscratch/zlu/dataset/imagenet/eig_vecs_train/dino_s16
    for i, file in enumerate(batch_files):
        
        file_name = file.split(".")[0]
        if args.split == "train":
            file_name = file_name.replace('/train/JPEGImages/',save_directory)
        elif args.split == "val":
            file_name = file_name.replace('/train/',save_directory)
            #file_name = file_name.replace('/val/',save_directory)
        file_name = file_name+'.pt'
        
        foldername = os.path.split(file_name)[0]
        if not os.path.exists(foldername):
            os.mkdir(foldername)
        #print(file_name)
        torch.save(eigenvectors[i].detach().cpu().clone(), file_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-root", type=str, default="/netscratch/zlu/dataset/imagenet/", help="Path to coco dataset")
    parser.add_argument("--split", type=str, default="val", choices=["train", "val"], help="Dataset split")
    parser.add_argument("--output-dir", type=str, default="/netscratch/zlu/dataset/imagenet/eig_vecs_train", help="Output directory for json results evaluation file")
