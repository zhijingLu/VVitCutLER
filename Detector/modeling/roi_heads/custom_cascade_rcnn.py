...
class CustomCascadeROIHeads(CustomStandardROIHeads):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.aggregation_module = SelsaAggregator(in_channels=256,num_attention_blocks=16)
        ...
        for match_iou, bbox_reg_weights in zip(cascade_ious, cascade_bbox_reg_weights):
            ...
            box_predictors.append(
                FastRCNNOutputLayers(
                    ...
                    #aggregation_module=self.aggregation_module,  
                )
            )
    ...
    def forward(self, images, features, proposals, file_name,targets=None): 
        del images
        if self.training:
            #proposals = self.label_and_sample_proposals_topk(proposals, targets, 150)
    ...
    def _forward_box(self, features, proposals, targets=None):
        ...
        for k in range(self.num_cascade_stages):
            ...
            if self.training:       
                for x in proposals:
                    if x.proposal_boxes.tensor.size(0)< topk:
                        pad_proposal = torch.zeros(topk,x.proposal_boxes.tensor.size(1)).to('cuda')
                        pad_proposal[:x.proposal_boxes.tensor.size(0), :] = x.proposal_boxes.tensor
                        x.proposal_boxes.tensor = pad_proposal.to('cuda') 
                    if self.training and x.gt_classes.size(0) < topk:
                        pad_class = torch.ones(topk).to('cuda')
                        pad_class[:x.gt_classes.size(0)] = x.gt_classes
                        x.gt_classes= pad_class.long().to('cuda')
                    
                    if self.training and x.gt_boxes.tensor.size(0) < topk:
                        pad_gt = torch.zeros(topk,x.gt_boxes.tensor.size(1)).to('cuda')
                        pad_gt[:x.gt_boxes.tensor.size(0), :] = x.gt_boxes.tensor
                        x.gt_boxes.tensor = pad_gt       
            else:
                for x in proposals:
                        if x.proposal_boxes.tensor.size(0) > topk:
                            x.proposal_boxes.tensor = x.proposal_boxes.tensor[:topk]
                        if "objectness_logits" in x.get_fields():
                            if x.objectness_logits.size(0)>topk:
                                x.objectness_logits = x.objectness_logits[:topk]
            
                
            
            predictions = self._run_stage(features, proposals, k,self.num_cascade_stages)
    ...
    def aggregator_frame(self, box_features, num_frames, num_proposals, spatial_shape):
        B, N, C, H, W = num_frames, num_proposals, box_features.shape[1], *spatial_shape

        box_features = box_features.view(B, N, C, 7, 7)
        pooled = box_features.mean(dim=[3, 4])  # [B, N, C]

        agg_list = []
        for i in range(B):
            key = pooled[i]  # [N, C]
            ref = torch.cat([pooled[j] for j in range(B) if j != i], dim=0)  # [(B-1)*N, C]
            out = self.aggregation_module(key, ref)
            key =key + out
            refined = self.inplace_false_relu(key)
            agg_list.append(refined)
        agg_features = torch.stack(agg_list, dim=0).view(B*N, C)
        agg_features = agg_features.view(B * N, C, 1, 1)
        agg_features = agg_features.expand(-1, -1, H, W) 
        return agg_features

    def _run_stage(self, features, proposals, stage,total_stage):
        """
        Args:
        ...
        box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])#[4096, 256, 7, 7])
        spatial_shape = box_features.shape[-2:]
        # The original implementation averages the losses among heads,
        # but scale up the parameter gradients of the heads.
        # This is equivalent to adding the losses among heads,
        # but scale down the gradients on features.
        if self.aggregation_module is not None:
            box_features = self.aggregator_frame(box_features, num_frames=8, num_proposals=512,spatial_shape=spatial_shape)  # 你定义的函数
        ...
        #print('######box_features',box_features.size())
        #assert id(self.box_predictor[0].aggregator) == id(self.box_predictor[1].aggregator)

        return self.box_predictor[stage](box_features,stage,total_stage,self.training,512,8)
